...
class DSAutoAdvisorPipeline:
    ...
    def _execute_profiling_stage_core(self) -> StageResult:
        ...
        try:
            ...
            # Import and run profiling (using dynamic import for numbered module)
            import importlib.util
            import sys
            from pathlib import Path
            
            # Dynamic import for numbered module
            module_path = Path(__file__).parent.parent / "src" / "1_data_profiling" / "data_profiler.py"
            spec = importlib.util.spec_from_file_location("data_profiler", module_path)
            profiler_module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(profiler_module)
            run_profiler = profiler_module.main
    ...
    def _execute_cleaning_stage_core(self) -> StageResult:
        ...
        try:
            # Import and run cleaning (using dynamic import for numbered module)
            import importlib.util
            from pathlib import Path
            
            # Dynamic import for numbered module
            module_path = Path(__file__).parent.parent / "src" / "2_data_cleaning" / "data_cleaner.py"
            ...
            CleaningConfig = cleaner_module.CleaningConfig
    ...
    def _execute_advisory_stage_core(self) -> StageResult:
        ...
        try:
            ...
            # Import assumption checker
            module_path = Path(__file__).parent.parent / "src" / "3_advisory" / "assumption_checker.py"
            spec = importlib.util.spec_from_file_location("assumption_checker", module_path)
            checker_module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(checker_module)
            EnhancedAssumptionChecker = checker_module.EnhancedAssumptionChecker
            AssumptionConfig = checker_module.AssumptionConfig
            
            # Import model recommender
            module_path = Path(__file__).parent.parent / "src" / "3_advisory" / "model_recommender.py"
            spec = importlib.util.spec_from_file_location("model_recommender", module_path)
            recommender_module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(recommender_module)
            recommend_model = recommender_module.recommend_model
            infer_target_type = recommender_module.infer_target_type
    ...
    def _execute_training_stage_core(self) -> StageResult:
        ...
        try:
            ...
            module_path = Path(__file__).parent.parent / "src" / "4_model_training" / "trainer.py"
            spec = importlib.util.spec_from_file_location("trainer", module_path)
            trainer_module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(trainer_module)
            EnhancedModelTrainer = trainer_module.EnhancedModelTrainer
    ...
    def _execute_evaluation_stage_core(self) -> StageResult:
        ...
        try:
            ...
            module_path = Path(__file__).parent.parent / "src" / "5_evaluation" / "evaluator.py"
            spec = importlib.util.spec_from_file_location("evaluator", module_path)
            evaluator_module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(evaluator_module)
            ModelAnalyzer = evaluator_module.ModelAnalyzer
            AnalysisConfig = evaluator_module.AnalysisConfig
